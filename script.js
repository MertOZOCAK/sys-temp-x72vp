...
function loadTrack(index) {
    ...
    curr_video.src = music_list[index].video || '';
...
function playTrack() {
    if (isVideoMode && curr_video.src) curr_video.play();
    curr_track.play();
    isPlaying = true;
...
function seekTo() {
    // Eğer video modundaysak VE bir video dosyası yüklüyse video süresini baz al, 
    // aksi takdirde her zaman müzik süresini baz al.
    let active = (isVideoMode && music_list[track_index].video) ? curr_video : curr_track;
    
    let seekto = active.duration * (seek_slider.value / 100);
    
    // Hem müziği hem videoyu aynı ana getir ki senkron bozulmasın
    curr_track.currentTime = seekto;
    if (curr_video.src) curr_video.currentTime = seekto;
}
...
function setMediaType(type) {
    ...
    if (type === 'video') {
        ...
        if (!music_list[track_index].video) {
            videoMsg.style.display = 'flex'; // Siyah ekran ve yazıyı göster
            curr_video.style.display = 'none'; // Video elementini gizle
        } else {
            videoMsg.style.display = 'none'; // Mesajı gizle
            curr_video.style.display = 'block'; // Videoyu göster
            curr_video.currentTime = curr_track.currentTime;
        }
    } else {
        isVideoMode = false;
