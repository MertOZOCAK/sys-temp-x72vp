...
let wave = document.getElementById('wave'); // Çizgili Efekt Konteynırı
...
// Liste (Sadeleştirildi, mp3 ve resimlerini buraya ekle)
const music_list = [
...
// DÜZELTİLEN OYNATMA FONKSİYONU
function playTrack() {
    curr_track.play();
    isPlaying = true;
    
    // Hem neon parlamayı hem de çizgili efekti tetikler
    track_art.classList.add('neon-glow'); 
    wave.classList.add('loader'); // 'wave' (id) buraya düzgün şekilde ekleniyor.
    
    playpause_btn.innerHTML = '<i class="fa fa-pause-circle fa-5x"></i>';
}

// DÜZELTİLEN DURDURMA FONKSİYONU
function pauseTrack() {
    curr_track.pause();
    isPlaying = false;
    
    // Hem neon parlamayı hem de çizgili efekti durdurur
    track_art.classList.remove('neon-glow'); 
    wave.classList.remove('loader'); // 'wave' (id) buraya düzgün şekilde kaldırılıyor.
...
function setUpdate() {
    if (!isNaN(curr_track.duration)) {
        let seekPos = curr_track.currentTime * (100 / curr_track.duration);
        seek_slider.value = seekPos;
        let curM = Math.floor(curr_track.currentTime / 60), curS = Math.floor(curr_track.currentTime % 60);
        let durM = Math.floor(curr_track.duration / 60), durS = Math.floor(curr_track.duration % 60);
