...
const music_list = [
    ...
    { img: 'images/logo.png', name: 'Sen Gideli', artist: 'Kubat', music: 'music/SenGideli.mp3' },
    { img: 'images/logo.png', name: 'Çalın Davulları', artist: 'Kubat', music: 'music/ÇalınDavulları.mp3' },
    { img: 'images/logo.png', name: 'Gönlüm Ataşlara Yandı', artist: 'Kubat', music: 'music/GönlümAtaşlaraYandı.mp3' },
    { img: 'images/logo.png', name: 'Kiraz Dalı', artist: 'Kubat', music: 'music/KirazDalı.mp3' },
    { img: 'images/logo.png', name: 'Ayletme Beni', artist: 'Kubat', music: 'music/AyletmeBeni.mp3' },
    ...
    { img: 'images/logo.png', name: 'Eller Üzer', artist: 'Serkan Nişancı', music:'music/EllerÜzer.mp3' },
...
function loadTrack(index) {
    ...
    // resetValues(); <--- Bu satırı sil veya yorum satırı yap
    ...
    // track_index'i burada güncelle ki saveCurrentState doğru indeksi alabilsin
    track_index = index; 
    now_playing.textContent = (index + 1) + " / " + music_list.length + " ŞARKIDA ÇALINIYOR";

    // Mevcut onended satırlarından hemen sonra şunu ekle:
    curr_track.onloadedmetadata = null; // Eski dinleyicileri temizle
...
function randomTrack() { isRandom = !isRandom; randomIcon.classList.toggle('randomActive', isRandom); saveCurrentState();}
function repeatTrack() { isRepeat = !isRepeat; repeatIcon.classList.toggle('repeatActive', isRepeat); saveCurrentState(); }
...
function setUpdate() {
    ...
    if (!isNaN(active.duration) && active.duration > 0) {
        ...
        // Sadece oynatılıyorsa kaydet (Duraklatıldığında veya yüklenirken hatalı saniye kaydetmemesi için)
        if (!active.paused) {
            saveCurrentState();
        }
    }
...
function saveCurrentState() {
    ...
    localStorage.setItem('novaStreamState', JSON.stringify(state));
}

function loadSavedState() {
    let savedState = localStorage.getItem('novaStreamState');
    
    if (savedState) {
        let state = JSON.parse(savedState);
        
        // Önce değişkenleri güncelle
        track_index = parseInt(state.index) || 0; 
        isRandom = state.isRandom;
        isRepeat = state.isRepeat;
        volume_slider.value = state.volume || 50;
        setVolume();

        // Şimdi şarkıyı yükle
        loadTrack(track_index);
        
        // Şarkı teknik olarak hazır olduğunda saniyeyi ata
        curr_track.onloadedmetadata = () => {
            if (state.currentTime > 0) {
                curr_track.currentTime = state.currentTime;
                setUpdate(); 
            }
        };

        // İkonları görsel olarak güncelle
        if (isRandom) randomIcon.classList.add('randomActive');
        if (isRepeat) repeatIcon.classList.add('repeatActive');
    } else {
        // Eğer hiçbir kayıt yoksa (ilk kez giriliyorsa) 0'dan başla
        loadTrack(0);
    }
}

// Sayfa tamamen yüklendiğinde hafızadaki durumu yükle
